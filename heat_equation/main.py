...
def create_A(r, N):
    ...
    diag1 = np.tile(np.concatenate(
        (np.array([0.0]), np.array([-r]), np.full(N - 2, -r / 2.))), N)
    diag_1 = np.tile(np.concatenate(
        (np.full(N - 2, -r / 2.), np.array([-r]), np.array([0.0]))), N)

    diag_N = np.full((N * N), -1 * r / 2.)
    # diag_N[1, ::] = np.full(N * 1, -r)
    
    A = spdiags([diag_N.flip(), diag_1, diag_0, diag1, diag], [-N - 1, -1, 0, 1, N], N ** 2, N ** 2)
    # A =
    A.tocsr()
